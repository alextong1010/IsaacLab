...
RESUME="True"
LOAD_RUN="2025-03-10_13-55-55"  # Replace with your actual run folder name
LOAD_CHECKPOINT="model_9999.pt"  # Replace with the iteration number to load
...
./isaaclab.sh -p "$SCRIPT_PATH" --task "$TASK" --num_envs "$NUM_ENVS" --seed "$SEED" \
  ...
  --checkpoint "$LOAD_CHECKPOINT" $ONLY_POSITIVE_REWARDS